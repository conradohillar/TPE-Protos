...
void access_register_add_entry(access_register_t* reg, const char* username, const char* destination, time_t timestamp) {
    ...
    LOG(DEBUG, "Access entry added: user=%s, destination=%s", username, destination);
}
...
size_t access_register_print(const access_register_t* reg, char* response, size_t response_size) {
    ...
    size_t idx = reg->head;
    size_t written = 0;
    int n = snprintf(response + written, response_size - written, "%-20s %-25s "
                                                                  "%-20s\n----------------------------------------------------"
                                                                  "--------------\n",
                     "Usuario",
                     "Destino",
    ...
    for (size_t i = 0; i < reg->count; i++) {
        ...
        n = snprintf(response + written, response_size - written, "%-20s %-25s %-20s\n", entry->username, entry->destination, timebuf);
        if (n < 0 || (size_t) n >= response_size - written)
            break;
        written += n;
