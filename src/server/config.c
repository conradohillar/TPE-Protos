...
void config_process_command(config_cmd_parsed_t *parsed_cmd, char *response,
  ...
  case CMD_REMOVE_USER:
    if (auth_remove_user(parsed_cmd->arg1)) {
      ...
      LOG(WARNING, "Failed to remove user (does not exist): %s",
          parsed_cmd->arg1);
  ...
  case CMD_GET_ACCESS_REGISTER:
    ...
    access_register_print(get_server_data()->access_register, response,
                          response_size);
    break;
